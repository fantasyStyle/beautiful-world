...
static NSString *appkId=@"12ew23ef43rfsd34rwefsd";

static NSString *appscret=@"1q2w3e4r5t6y7u8i";
...
+ (NSString *)AES256EncryptWithPlainText:(NSString *)plain  randomText:(NSString *)randomText {
    ...
    /**********自定义向量,MD5加密,排序*************/
    NSMutableArray *sortAry = [NSMutableArray arrayWithObjects:appkId,appscret,randomText, nil];
    NSArray * sortedAry= [sortAry sortedArrayUsingSelector:@selector(compare:)];
//    NSMutableArray *reSortArray= [[NSMutableArray alloc] init];
//    for (id _obj in [sortedAry reverseObjectEnumerator]) {
//        [reSortArray addObject:_obj];
//    }
    
    __block NSString *ivString = @"";
    [sortedAry enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ivString = [ivString stringByAppendingString:obj];
    }];
    ivString = [[ivString md5] substringWithRange:NSMakeRange(8, 16)];;
    const void *ivByte = [[ivString dataUsingEncoding:NSUTF8StringEncoding] bytes];
    /**********自定义向量,MD5加密,排序*************/
    
	CCCryptorStatus cryptStatus = CCCrypt(kCCEncrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding,
                                          [[NSData AESKeyForPassword:appkId] bytes], kCCKeySizeAES256,
